    ...
    this.router.post("/", this.ensureAuthenticated, csrfProtection, async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          // Modern shadcn-style scaffold: hsl() semantic tokens, no hardcoded
          // #667eea/#764ba2 purple-pink gradient (the "forbidden pattern" in
          // MODERN_DESIGN_PROMPT), no purple→indigo hero gradient, neutral
          // palette with first-class dark mode that the user can extend.
          // Tailwind CDN is still used here because these starter files are
          // a sandbox HTML page — the user can graduate to local PostCSS
          // when they convert to a real React/Vite project.
          type StarterFile = { name: string; content: string };
          const modernHead = (title: string) => `  <meta charset="UTF-8">\n  <meta name="viewport" content="width=device-width, initial-scale=1.0">\n  <title>${title}</title>\n  <script src="https://cdn.tailwindcss.com"><\/script>\n  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">\n  <script>\n    tailwind.config = {\n      darkMode: 'class',\n      theme: {\n        extend: {\n          fontFamily: { sans: ['Inter', 'sans-serif'] },\n          colors: {\n            border: 'hsl(var(--border))',\n            background: 'hsl(var(--background))',\n            foreground: 'hsl(var(--foreground))',\n            primary: { DEFAULT: 'hsl(var(--primary))', foreground: 'hsl(var(--primary-foreground))' },\n            muted: { DEFAULT: 'hsl(var(--muted))', foreground: 'hsl(var(--muted-foreground))' },\n            accent: { DEFAULT: 'hsl(var(--accent))', foreground: 'hsl(var(--accent-foreground))' },\n            card: { DEFAULT: 'hsl(var(--card))', foreground: 'hsl(var(--card-foreground))' },\n          },\n        },\n      },\n    };\n  <\/script>\n  <style>\n    :root { --background: 0 0% 100%; --foreground: 240 10% 3.9%; --card: 0 0% 100%; --card-foreground: 240 10% 3.9%; --primary: 240 5.9% 10%; --primary-foreground: 0 0% 98%; --muted: 240 4.8% 95.9%; --muted-foreground: 240 3.8% 46.1%; --accent: 240 4.8% 95.9%; --accent-foreground: 240 5.9% 10%; --border: 240 5.9% 90%; }\n    .dark { --background: 240 10% 3.9%; --foreground: 0 0% 98%; --card: 240 10% 3.9%; --card-foreground: 0 0% 98%; --primary: 0 0% 98%; --primary-foreground: 240 5.9% 10%; --muted: 240 3.7% 15.9%; --muted-foreground: 240 5% 64.9%; --accent: 240 3.7% 15.9%; --accent-foreground: 0 0% 98%; --border: 240 3.7% 15.9%; }\n    body { font-family: 'Inter', sans-serif; }\n  </style>`;

          const modernHero = (title: string, editFile: string) => `<body class="min-h-screen bg-background text-foreground antialiased">\n  <header class="border-b border-border/40 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">\n    <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">\n      <span class="text-sm font-semibold tracking-tight">${title}</span>\n      <button id="theme-toggle" class="text-xs text-muted-foreground hover:text-foreground transition-colors">Toggle theme</button>\n    </div>\n  </header>\n  <main class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-24 sm:py-32">\n    <h1 class="text-4xl font-bold tracking-tight text-balance sm:text-5xl">${title}</h1>\n    <p class="mt-4 text-base leading-7 text-muted-foreground text-balance">A clean starting point. Edit <code class="rounded-md border border-border/60 bg-muted px-1.5 py-0.5 font-mono text-sm">${editFile}</code> to begin building.</p>\n    <div class="mt-8 flex items-center gap-3">\n      <a href="#" class="inline-flex h-9 items-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">Get started</a>\n      <a href="https://github.com/openaxcloud/Vibe-Companion" target="_blank" rel="noopener" class="inline-flex h-9 items-center rounded-md border border-border bg-background px-4 text-sm font-medium hover:bg-accent transition-colors">Documentation</a>\n    </div>\n  </main>\n  <script>\n    document.getElementById('theme-toggle')?.addEventListener('click', () => {\n      document.documentElement.classList.toggle('dark');\n    });\n  <\/script>`;

          const multiFileStarters: Record<string, StarterFile[]> = {
            javascript: [
              { name: 'index.html', content: `<!DOCTYPE html>\n<html lang="en">\n<head>\n${modernHead(projectName)}\n</head>\n${modernHero(projectName, 'script.js')}\n  <script src="script.js"><\/script>\n</body>\n</html>\n` },
              { name: 'script.js', content: `// ${projectName}\nconsole.log('Hello, World!');\n` },
            ],
            typescript: [
              { name: 'index.html', content: `<!DOCTYPE html>\n<html lang="en">\n<head>\n${modernHead(projectName)}\n</head>\n${modernHero(projectName, 'index.ts')}\n</body>\n</html>\n` },
              { name: 'index.ts', content: `// ${projectName}\nconsole.log('Hello, World!');\n` },
            ],
            html: [
              { name: 'index.html', content: `<!DOCTYPE html>\n<html lang="en">\n<head>\n${modernHead(projectName)}\n</head>\n${modernHero(projectName, 'script.js')}\n  <script src="script.js"><\/script>\n</body>\n</html>\n` },
              { name: 'style.css', content: `@tailwind base;\n@tailwind components;\n@tailwind utilities;\n` },
