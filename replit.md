...
- **Command Palette Wiring**: UnifiedIDELayout wires `onNewFile`/`onNewFolder` via custom DOM events (`ecode:new-file`, `ecode:new-folder`). ReplitFileExplorer listens for these events to open its internal dialogs. Platform-aware shortcut display uses `platformShortcut()` in `CommandPalette.tsx` — shows `⌘/⌥/⇧` on Mac, `Ctrl/Alt/Shift` on Windows/Linux.
