...
- **AI Agent System Prompt**: Agent prompt in `legacy-ai-assistant.ts` (~line 2598) instructs AI to build production-ready, beautifully designed apps. Includes mandatory architecture rules (React+Vite+Tailwind for non-trivial apps), comprehensive design system (dark mode, glassmorphism, typography, animations, responsive), and strict completeness requirements. Output quality is a top priority — apps must look like premium SaaS products. Memory Bank context (project brief, architecture, active context) is automatically injected at the end of the system prompt.
- **Agent Conversation Persistence**: The `/api/ai/agent` endpoint now intercepts `res.write` SSE events to accumulate agent text and file operations during streaming. After streaming completes, it creates/retrieves a conversation and saves both the user message and the full assistant response (capped at 100KB) with file operation metadata. This enables conversation history to persist across sessions.
- **Memory Bank**: `server/services/memory-bank.service.ts` provides per-project persistent context files stored in `project-workspaces/{projectId}/.ecode/memory-bank/`. Auto-initialized with AI-generated content on project bootstrap (via `workspace-bootstrap.router.ts`). Files: `projectbrief.md`, `productContext.md`, `systemPatterns.md`, `techContext.md`, `activeContext.md`. Context injected into both `/api/ai/agent` and `/api/ai/chat` system prompts. `activeContext.md` auto-updated after agent file modifications. All project ID params accept `string | number`.
...
- **Preview route**: Working path is `/api/preview/projects/:id/preview/`. Legacy `legacy-ai-assistant.ts` has catch-all routes at `/api/preview/:projectId/{*path}` that skip `projectId === 'projects'` to allow the modular preview router to handle the `/projects/` prefix. The preview router (`server/routes/preview.ts`) uses its own `requireAuth` middleware (session + passport compatible) and `ensureProjectAccess` (checks `project.userId || project.ownerId`). File schema uses `f.filename` — use `fname(f)` helper. Runnable file detection uses `hasRunnableFiles()` which checks 30+ extensions (.go, .rb, .rs, .java, .cpp, .php, etc.) and build system files (Cargo.toml, go.mod, Gemfile, pom.xml, etc.). Static-only detection (HTML without server-side code) is inline in the preview URL endpoint.
...
- **Platform Utilities**: `client/src/lib/platform.ts` exports `isMac`, `platformShortcut(keys)`, and `platformKey(key)` for OS-aware keyboard display (⌘/⌥/⇧ on Mac, Ctrl/Alt/Shift on Windows/Linux). Used by CommandPalette, KeyboardShortcutsOverlay, and any future shortcut display.
- **Command Palette Wiring**: UnifiedIDELayout wires `onNewFile`/`onNewFolder` via custom DOM events (`ecode:new-file`, `ecode:new-folder`). ReplitFileExplorer listens for these events to open its internal dialogs.
- **Keyboard Shortcuts in UnifiedIDELayout**: All keyboard shortcuts use `matchesCommand()` from `useKeyboardShortcuts` hook instead of hardcoded `Ctrl/Meta` checks. Supports: command palette, quick file search, global search, sidebar toggle, terminal, preview, new file, run/stop, git panel, keyboard shortcuts overlay. All shortcuts respect user customizations.
- **Batch File Delete**: `POST /api/files/batch-delete` with `{ ids: string[] }` — atomic deletion using `inArray` Drizzle query. Pre-validates all files belong to same project with write access. Frontend uses this for folder deletion instead of sequential individual deletes.
- **Project Archive**: `GET /api/projects/:id/export` streams ZIP of all project files. `POST /api/projects/:id/import` (multipart `files` field) imports uploaded files into a project. Router: `server/routes/project-archive.router.ts`. Frontend Import/Export panel in ToolsDropdown triggers `ecode:files-changed` event after import so file explorer refreshes.
- **Inline Rename**: ReplitFileExplorer supports inline rename via context menu → "Rename". The rename preserves directory prefixes (e.g. `src/App.tsx` → `src/NewName.tsx`). State: `inlineRenameId` controls which tree item shows the input.
- **Progress Panel**: Fetches real tasks from `/api/projects/:id/tasks` and workflows from `/api/workflows?projectId=...` with 10s polling. Shows running workflows, active tasks, and completed tasks.
- **Session Replay Panel**: Fetches checkpoints from `/api/projects/:id/checkpoints` and displays them as a vertical timeline with expandable details.
- **Deployment Pipeline**: ToolsDropdown "deployment-pipeline" tab wired to `ReplitDeploymentPipeline` component with canvas-based CI/CD visualization.
- **MobileSearch**: Now calls real APIs (`/api/projects?search=`, `/api/marketplace/templates?search=`, `/api/users/search?q=`) and shows actual search results with icons and badges instead of skeleton placeholders.
- **Minimap Visible Range**: MultiTabEditor now tracks `visibleRange` from CM6 `geometryChanged` events and passes it to ReplitMinimap for viewport overlay rendering.
- **ToolsDropdown**: All 31 tool IDs map to render cases in UnifiedIDELayout. Search input filters by label/description. Each item shows description. IDs like `authentication`→ReplitAuthPanel, `install`/`package-explorer`→ReplitPackagesPanel, `extension-store`→ExtensionsMarketplace, `user-settings`→ReplitSettingsPanel, `version-control`/`fork-graph`→ReplitGitPanel, `deployments`→ReplitDeploymentPanel, `code-search`→GlobalSearch, `docs`→iframe, `object-storage`→ObjectStorage, `replit-key-value`→DatabasePanel, `vnc`→placeholder, `resource-monitor`→ResourceMonitor, `agent`/`assistant`→ReplitAgentPanelV3.
- **AI Model Selector**: AIModelSelector supports inline/card/hero variants. AIPanel has built-in model switcher (GPT-4.1, Claude Sonnet 4, Gemini 2.5, etc.) with fallback chain. Agent panel passes through to AIPanel. Models grouped by provider (Anthropic, OpenAI, Google, External Agents) with visual separators. `specificModelId` state tracks exact model selection, sent in request body as `modelId` across main send, retry, and plan-start paths. Backend `resolveTopAgentMode()` validates `explicitModelId` against allowlist of known models before overriding. Display shows specific model name (e.g. "Claude Sonnet 4") instead of group name.
- **ResponsiveWebPreview**: Full browser-like preview with editable URL bar, back/forward navigation history, device simulation (iPhone 14, SE, Pixel 7, iPad, iPad Pro), auto-start on mount, status polling, Play/Stop/Refresh buttons, green/yellow/red status dot, loading overlay, error state with retry, console panel with injected bridge script that captures `console.log/warn/error/info` and uncaught errors from the iframe, fullscreen support with proper `fullscreenchange` listener.
...
- **Branding**: All user-facing text uses "E-Code" brand. No remaining "Vibe Companion" or "Vibe platform" references. Internal component prefixes like `Replit*` are kept as-is (code-level, not user-facing).
