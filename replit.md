...
- **AI Agent**: Tool-use endpoint that can create/edit files, create skills, and (when web search enabled) search the web and fetch URL content directly in the project. Web search uses DuckDuckGo for real-time results; `fetch_url` has SSRF protection (blocks private/internal IPs). Search results rendered as citation cards with clickable source links. Message Queue system allows users to queue follow-up messages during streaming, with auto-processing loop, drag-and-drop reorder, inline edit, pause/resume, and DB persistence (`queued_messages` table).
