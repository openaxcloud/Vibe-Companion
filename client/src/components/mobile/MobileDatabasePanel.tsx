...
export function MobileDatabasePanel({ projectId, className }: MobileDatabasePanelProps) {
  ...
  const { data: credentials, isLoading: credentialsLoading, refetch: refetchCredentials } = useQuery<DatabaseCredentials>({
    queryKey: ['/api/projects', projectId, 'database', 'credentials'],
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/projects/${projectId}/database/credentials?env=development`);
      return response;
    },
    staleTime: 60000,
  ...
  const provisionMutation = useMutation({
    mutationFn: async (data: ProvisionRequest) => {
      // Provision via execute endpoint with init script
      return apiRequest('POST', `/api/projects/${projectId}/database/execute`, {
        sql: `SELECT 1; -- Placeholder for provision with plan: ${data.plan}, region: ${data.region}`
      });
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiRequest('POST', `/api/projects/${projectId}/database/remove`, {});
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'database'] });
      toast({
  ...
  const { data: backupsData, isLoading: backupsLoading, refetch: refetchBackups } = useQuery<BackupsResponse>({
    queryKey: ['/api/projects', projectId, 'database', 'backups'],
    queryFn: async () => {
      // Backups endpoint not available in backend - return empty response
      toast({
        title: 'Feature Coming Soon',
        description: 'Backup management will be available soon.',
      });
      return { backups: [] };
    },
    staleTime: 30000,
  ...
  const createBackupMutation = useMutation({
    mutationFn: async (data: { name?: string; backupType?: string }) => {
      // Backups endpoint not available in backend - show coming soon
      throw new Error('Backup creation coming soon');
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'database', 'backups'] });
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'database'] });
      toast({
    ...
    onError: (error: any) => {
      toast({
        title: 'Feature Coming Soon',
        description: 'Backup creation will be available soon.',
        variant: 'default',
      });
    },
  });
  ...
  const restoreBackupMutation = useMutation({
    mutationFn: async (backupId: number) => {
      // Restore endpoint not available in backend - show coming soon
      throw new Error('Restore coming soon');
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'database', 'backups'] });
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'database'] });
  ...
  const deleteBackupMutation = useMutation({
    mutationFn: async (backupId: number) => {
      // Delete backup endpoint not available in backend - show coming soon
      throw new Error('Delete backup coming soon');
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'database', 'backups'] });
      toast({
    ...
    onError: (error: any) => {
      toast({
        title: 'Feature Coming Soon',
        description: 'Backup deletion will be available soon.',
        variant: 'default',
      });
    },
  });
  ...
  const { data: tableData, isLoading: dataLoading } = useQuery<TableDataResponse>({
    queryKey: ['/api/projects', projectId, 'database', 'tables', selectedTable, 'page', currentPage],
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/projects/${projectId}/database/tables/${selectedTable}/data?page=${currentPage}&limit=100`);
      return response;
    },
  ...
  const schemasQueries = useQueries({
    queries: allTables.map(table => ({
      queryKey: ['/api/projects', projectId, 'database', 'tables', table.name, 'data'],
      queryFn: async () => {
        // Schema is included in table data response, so fetch the data endpoint
        const response = await apiRequest('GET', `/api/projects/${projectId}/database/tables/${table.name}/data?page=1&limit=1`);
        return response;
      },
