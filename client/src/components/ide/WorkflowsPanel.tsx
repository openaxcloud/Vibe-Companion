...
export function WorkflowsPanel({ projectId, onRunWorkflow, className }: WorkflowsPanelProps) {
  ...
  const stopMutation = useMutation({
    mutationFn: async (workflowId: string) => {
      return apiRequest('POST', `/api/workflows/${workflowId}/stop`, {});
    },
    onSuccess: () => {
  ...
  const WorkflowCard = ({ workflow }: { workflow: Workflow }) => {
    ...
    const isPending = runMutation.isPending && runMutation.variables?.id === workflow.id;
    ...
    return (
      <Card className={cn(
        ...
        isRunning && "ring-1 ring-green-500/30",
        ...
        <CardContent className="p-3">
          <div className="flex items-start gap-3">
            <div className={cn(
              ...
              <Icon className={cn("h-4 w-4", workflow.isDefault && "text-primary", isRunning && "text-green-500")} />
              {isRunning && (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {isRunning && (
                  <Badge className="text-[10px] h-4 bg-green-600">Running</Badge>
                )}
                {isCompleted && (
                  <Badge className="text-[10px] h-4 bg-blue-600">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <code className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded inline-block">
                  {workflow.command}
                </code>
                {workflow.runCount && workflow.runCount > 0 && (
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              {isRunning ? (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => stopMutation.mutate(workflow.id)}
