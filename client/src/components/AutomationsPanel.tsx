...
export default function AutomationsPanel({ projectId, onClose }: AutomationsPanelProps) {
  ...
  const [newType, setNewType] = useState<"cron" | "webhook" | "on-deploy">("cron");
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", `/api/projects/${projectId}/automations`, {
        name: newName, type: newType, cronExpression: newType === "cron" ? newCron : undefined,
        script: newScript, language: newLanguage,
      });
      return res.json();
    },
    onSuccess: () => {
      ...
      {creating && (
        <div className="px-3 py-2 border-b border-[var(--ide-border)] space-y-2">
          ...
          <div className="flex gap-1">
            {(["cron", "webhook", "on-deploy"] as const).map((t) => (
              <button key={t} onClick={() => setNewType(t)}
                className={`flex-1 text-[10px] py-1.5 rounded-md border transition-colors ${newType === t ? "bg-[#F5A623]/10 border-[#F5A623]/30 text-[#F5A623]" : "border-[var(--ide-border)] text-[var(--ide-text-muted)] hover:text-[var(--ide-text)]"}`}
                data-testid={`button-type-${t}`}>
                {t === "cron" ? "Cron" : t === "webhook" ? "Webhook" : "On Deploy"}
              </button>
            ))}
          </div>
          {newType === "cron" && (
          ...
          <div className="flex gap-1.5">
            <Button size="sm" className="h-7 px-3 text-[10px] bg-[#F5A623] hover:bg-[#F5A623]/80 text-black rounded-md font-semibold" onClick={() => createMutation.mutate()} disabled={!newName.trim() || createMutation.isPending} data-testid="button-create-automation">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {automationsQuery.data && automationsQuery.data.length === 0 && !creating && (
          <div className="flex flex-col items-center justify-center py-8 text-center px-4">
            ...
            <p className="text-[10px] text-[var(--ide-text-muted)] mt-1 opacity-60">Create cron jobs, webhooks, or deploy triggers</p>
          </div>
        )}
                        ...
                        <div key={run.id} className="flex items-start gap-1.5 p-1.5 rounded bg-[var(--ide-bg)] border border-[var(--ide-border)]" data-testid={`automation-run-${run.id}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-[8px] text-[var(--ide-text-muted)]">via {run.triggeredBy}</span>
