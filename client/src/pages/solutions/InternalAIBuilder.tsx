    ...
    <MarketingLayout>
      <section className="relative overflow-hidden py-12 sm:py-16">
        <div className="container-responsive max-w-5xl text-center px-4 sm:px-6">
          <Badge className="mx-auto mb-4 sm:mb-6 bg-foreground/10 text-foreground border-foreground/20">Internal AI Builder</Badge>
          ...
          <p className="mt-3 sm:mt-4 text-[13px] sm:text-base md:text-[15px] text-muted-foreground px-2 sm:px-0">
          ...
          <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 px-4 sm:px-0">
            ...
            <Button 
              variant="outline" 
              className="border-border text-muted-foreground hover:text-foreground min-h-[44px] w-full sm:w-auto" 
      ...
      <section className="py-12 sm:py-16">
        <div className="container-responsive max-w-6xl grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 px-4 sm:px-6">
          {capabilities.map((capability, index) => (
            <Card key={capability.title} className="bg-muted/50 border-border" data-testid={`card-capability-${index}`}>
              <CardHeader className="space-y-3 sm:space-y-4 p-4 sm:p-6">
                <div className="inline-flex rounded-full bg-sky-100 dark:bg-sky-900/20 p-2 sm:p-3 text-sky-600 dark:text-sky-300">
                  <capability.icon className="h-5 w-5 sm:h-6 sm:w-6" />
                </div>
                <CardTitle className="text-base sm:text-[15px] text-foreground">{capability.title}</CardTitle>
                <CardDescription className="text-[13px] sm:text-base text-muted-foreground leading-relaxed">{capability.description}</CardDescription>
              </CardHeader>
            </Card>
          ))}
        </div>
          ...
          <Card className="bg-muted/50 border-border">
            <CardContent className="grid gap-6 sm:gap-8 py-8 sm:py-12 text-center sm:text-left grid-cols-1 sm:grid-cols-[1.5fr_1fr] p-4 sm:p-6">
              <div className="space-y-3 sm:space-y-4">
                <h2 className="text-2xl sm:text-3xl font-semibold text-foreground">Launch an internal AI center of excellence</h2>
