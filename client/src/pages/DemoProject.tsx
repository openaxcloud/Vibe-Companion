    ...
    <div className="h-screen flex flex-col bg-[#1C2333] text-sm select-none">
      <div className="flex items-center justify-between px-2 h-10 bg-[#1C2333] border-b border-[#2B3245] shrink-0">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" className="w-7 h-7 text-[#9DA2B0] hover:text-white hover:bg-[#2B3245]" onClick={() => setLocation("/")} data-testid="button-back-demo">
          ...
          <span className="text-xs font-semibold text-[#F5F9FC]">Demo Project</span>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <div className={`${isMobile ? "absolute left-0 top-10 bottom-0 z-30" : "relative"} w-[220px] bg-[#1C2333] border-r border-[#2B3245] flex flex-col shrink-0`}>
              ...
              <div className="flex-1 overflow-y-auto py-1">
                {demoQuery.data?.files?.map((file) => (
                  <div key={file.id} className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-colors ${file.id === activeFileId ? "bg-[#2B3245] text-white" : "text-[#F5F9FC] hover:bg-[#1C2333]"}`} onClick={() => openFile(file)} data-testid={`demo-file-${file.id}`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          {openTabs.length > 0 && (
            <div className="flex items-center bg-[#1C2333] border-b border-[#2B3245] overflow-x-auto shrink-0 scrollbar-hide">
              {openTabs.map((tabId) => {
                ...
                return (
                  <div key={tabId} className={`flex items-center gap-1.5 px-3 py-1.5 cursor-pointer border-r border-[#2B3245] shrink-0 transition-colors ${isActive ? "bg-[#1C2333] text-white border-t-2 border-t-[#0079F2]" : "text-[#9DA2B0] hover:text-[#F5F9FC] border-t-2 border-t-transparent"}`} onClick={() => openFile(file)}>
          ...
          {terminalVisible && (
            <div className="shrink-0 flex flex-col border-t border-[#2B3245] bg-[#1C2333]" style={{ height: 200 }}>
              <div className="flex items-center justify-between px-2 border-b border-[#2B3245] bg-[#1C2333] shrink-0">
                ...
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="icon" className="w-5 h-5 text-[#9DA2B0] hover:text-white hover:bg-[#2B3245]" onClick={() => setLogs([])}>
                    <RefreshCw className="w-3 h-3" />
                  </Button>
                  <Button variant="ghost" size="icon" className="w-5 h-5 text-[#9DA2B0] hover:text-white hover:bg-[#2B3245]" onClick={() => setTerminalVisible(false)}>
                    <X className="w-3 h-3" />
                  </Button>
                </div>
              </div>
              {bottomTab === "terminal" ? (
                <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-2 space-y-0.5" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "12px" }}>
                  {logs.map((log) => (
                    <div key={log.id} className={log.type === "error" ? "text-red-400" : log.type === "success" ? "text-green-400" : "text-[#9DA2B0]"}>
      ...
      {!terminalVisible && (
        <div className="flex items-center px-3 h-6 bg-[#1C2333] border-t border-[#2B3245] shrink-0">
          <button className="flex items-center gap-1 text-[10px] text-[#9DA2B0] hover:text-white" onClick={() => setTerminalVisible(true)}>
            <Terminal className="w-3 h-3" /> Console
          </button>
        </div>
      )}
